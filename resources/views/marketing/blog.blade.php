...
<section class="bg-navy text-white py-16">
    <div class="max-w-3xl mx-auto px-4 text-center">
        <h1 class="font-display text-4xl font-extrabold tracking-tight mb-3">The Esagio Blog</h1>
...
<section class="py-20 bg-gray-50">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <a href="{{ route('blog.post', $post->slug) }}" class="bg-white rounded-[5px] border border-gray-100 overflow-hidden hover:shadow-lg transition group">
                @if($post->featured_image_path)
                <img src="{{ Storage::url($post->featured_image_path) }}" alt="{{ $post->title }}" class="w-full h-48 object-cover">
                @else
                <div class="w-full h-48 bg-gradient-to-br from-navy/10 to-accent/10"></div>
                @endif
                <div class="p-6">
                    @if(!empty($post->tags))
                    <div class="flex flex-wrap gap-2 mb-3">
                        @foreach(array_slice($post->tags, 0, 2) as $tag)
                        <span class="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full font-mono">{{ $tag }}</span>
                        @endforeach
                    </div>
                    @endif
                    <h2 class="font-semibold text-gray-900 mb-2 group-hover:text-navy transition leading-snug">{{ $post->title }}</h2>
                    @if($post->excerpt)
