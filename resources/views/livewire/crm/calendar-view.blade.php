<div class="flex flex-col bg-white dark:bg-gray-900" style="height: calc(100vh - 48px);">
    ...
    <div class="flex items-center justify-between px-4 py-2 border-b border-gray-200 dark:border-gray-700 shrink-0">
        ...
        <div class="flex items-center gap-2">
            ...
            <h2 class="text-lg font-semibold text-gray-900 dark:text-white ml-1">{{ $this->headerLabel }}</h2>
        </div>
    </div>
    ...
    <div class="flex flex-col flex-1 min-h-0">
        {{-- Day headers --}}
        <div class="grid grid-cols-7 border-b border-gray-200 dark:border-gray-700 shrink-0">
        ...
        {{-- Day grid — each row stretches equally to fill all remaining space --}}
        @php
        ...
        <div class="flex flex-col flex-1 min-h-0">
            @foreach($weeks as $week)
                <div class="grid grid-cols-7 flex-1 min-h-0 border-b border-gray-200 dark:border-gray-700 last:border-b-0">
                    @foreach($week as $day)
                        <div
                            wire:click="openCreateModal('{{ $day['date'] }}')"
                            class="flex flex-col p-2 cursor-pointer transition-colors overflow-hidden
                                {{ !$loop->first ? 'border-l border-gray-200 dark:border-gray-700' : '' }}
                        ...
                        >
                            ...
                            @else
                                <span class="inline-flex items-center justify-center w-8 h-8 text-sm font-medium {{ $day['isCurrentMonth'] ? 'text-gray-800 dark:text-gray-200' : 'text-gray-400 dark:text-gray-600' }} shrink-0">
                                    {{ $day['dayNumber'] }}
                                </span>
                            @endif
                            ...
                            <div class="flex flex-col gap-0.5 mt-1 min-h-0 overflow-hidden flex-1">
                                ...
                                @if(count($day['appointments']) > 3)
                                    <div class="text-[10px] text-blue-500 dark:text-blue-400 font-medium px-1.5 shrink-0">+{{ count($day['appointments']) - 3 }} more</div>
                                @endif
                            </div>
                        </div>
                    @endforeach
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-md mx-4 border border-gray-200 dark:border-gray-700">
            ...
            <form wire:submit="createAppointment" class="p-5 space-y-4">
                <div>
                    ...
                    <input type="text" wire:model="title" class="w-full bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2.5 text-sm text-gray-900 dark:text-white placeholder-gray-400 focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="Add a title" autofocus>
                ...
                <div>
                    ...
                    <textarea wire:model="description" rows="2" class="w-full bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2.5 text-sm text-gray-900 dark:text-white placeholder-gray-400 focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="Add description..."></textarea>
                </div>
